...
RSpec.describe ElmHistoryTools::HistorySanitizer do
  describe ".sanitize_history" do
    context "simple example" do
      let(:history_data) {
        {"history" => [{"ctor" => "MySecretData", "_0" => "access_token", "_1" => {"password" => "myP@ssw0rd", "expiration" => "tomorrow"}}]}
      }
      ...
      it "sanitizes properly" do
        expect(ElmHistoryTools::HistorySanitizer.sanitize_history(history_data, watch_words, &sanitize)["history"]).to eq([{"ctor" => "MySecretData", "_0" => "[FILTERED]", "_1" => {"password" => "[FILTERED]", "expiration" => "tomorrow"}}])
      end
    end
